...
let res = await fetch(`https://api.nekorinn.my.id/anime/kusonime-search?q=${text}`)
...
let img = await prepareWAMessageMedia({ image: { url: anime.thumbnail } }, { upload: conn.waUploadToServer })
let genreList = anime.genres.join(', ')
let caption = `🍥 *${anime.title}*\n🍡 *Rilis: ${anime.releaseTime}*\n🍬 *Genre: ${genreList}*`
slides.push({
...
display_text: "🍭 Lihat di Kusonime",
url: anime.link,
...
body: { text: `🍰 *Hasil Pencarian Batch di Kusonime:* ${text}` },
...
console.error(e)
m.reply('❌ *Terjadi kesalahan saat mencari anime di Kusonime.*')
} finally {
