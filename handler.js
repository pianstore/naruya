...
❗ *Maaf, akses ke ${conn.user.name} melalui Private Chat tidak diperbolehkan.*
...
return m.reply(`❌ *Limit command kamu sudah habis!*
💡 *${user.command} / ${user.commandLimit} digunakan.*
⏳ *Silakan tunggu hingga reset limit di jam 00.00 WIB!*
${!user.verif ? "✨ *Atau verifikasi dulu dengan .verif untuk dapatkan bonus 5000 limit!*" : "🌟 *Upgrade ke Premium untuk Unlimited Limit!*"}
`).then(() => {
...
m.reply("*⛔ No Spam Commands! Silakan tunggu 1 Menit.*")
user.banned = true
...
this.adReply(m.chat, `*⚠️ Untuk menggunakan fitur ini, kamu harus mencapai level ${plugin.level}.*`, '⛔ AKSES DITOLAK', global.config.watermark, fs.readFileSync('./media/denied.jpg'), global.config.website, m, false)
continue
}
if (plugin.age > _user.age) {
this.adReply(m.chat, `*⚠️ Untuk menggunakan fitur ini, kamu harus berusia minimal ${plugin.age} tahun.*`, '⛔ AKSES DITOLAK', global.config.watermark, fs.readFileSync('./media/denied.jpg'), global.config.website, m, false)
continue
}
let extra = {
match,
...
this.adReply(id, text.trim(), "˚ ༘`✦ ˑ ִֶ 𓂃⊹ 𝗦𝗲𝗹𝗮𝗺𝗮𝘁 𝗗𝗮𝘁𝗮𝗻𝗴 𝗞𝗮𝗸", global.config.watermark, img, '', false, true, false, { smlcap: false })
}
break
}
case 'remove': {
...
text = text.replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'unknow').replace('@user', "@" + user.split("@")[0])
this.adReply(id, text.trim(), "˚ ༘`✦ ˑ ִֶ 𓂃⊹ 𝗦𝗲𝗹𝗮𝗺𝗮𝘁 𝗧𝗶𝗻𝗴𝗴𝗮𝗹 𝗞𝗮𝗸", global.config.watermark, img, '', false, true, false, { smlcap: false })
}
break
}
case 'promote': {
...
this.adReply(id, text, "˚ ༘`✦ ˑ ִֶ 𓂃⊹ 𝗨𝗽𝗱𝗮𝘁𝗲 𝗡𝗮𝗺𝗮 𝗚𝗿𝘂𝗽", global.config.watermark, img, '', false, true, false)
break
}
case 'desc': {
if (!chat.detect) return
...
this.adReply(id, text, "˚ ༘`✦ ˑ ִֶ 𓂃⊹ 𝗨𝗽𝗱𝗮𝘁𝗲 𝗗𝗲𝘀𝗸𝗿𝗶𝗽𝘀𝗶", global.config.watermark, img, '', false, true, false)
break
}
case 'icon': {
if (!chat.detect) return
let text = chat.sIcon || this.icon || conn.icon || 'Foto profil grup telah diubah.'
this.adReply(id, text, "˚ ༘`✦ ˑ ִֶ 𓂃⊹ 𝗨𝗽𝗱𝗮𝘁𝗲 𝗙𝗼𝘁𝗼 𝗚𝗿𝘂𝗽", global.config.watermark, img, '', false, true, false)
break
}
}
}
...
conn.adReply(m.chat, msg, '⛔ AKSES DITOLAK', global.config.watermark, fs.readFileSync('./media/denied.jpg'), global.config.website, m, false)
}
}
